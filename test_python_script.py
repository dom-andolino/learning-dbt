...
if __name__ == '__main__': 
    ...
    output_df = pd.DataFrame(columns=["celeb_nm", 'cat', 'cat_rnk']) #init empty df
    query_list = ['Actors', 'Netflix', 'NFL', 'YouTube', 'Bitcoin', 'Politician'] #TODO: store list in csv or DB
    ...
    output_df.sort_values(by = ['celeb_nm', 'cat_rnk', 'cat'], inplace = True)
    output_df.reset_index(drop=True, inplace=True)
    output_df['cat_rnk'] = output_df['cat_rnk'].apply(lambda x: int(x)) #needs to be an into for below transforms to work
    output_df['num_cats'] = output_df.groupby(['celeb_nm'])['cat'].transform('count') # count of rows (ie number of serach terms the name came up in)
    output_df['bst_cat_rnk'] = output_df.groupby(['celeb_nm'])['cat_rnk'].transform('min') # lowest rnk will be the best match
    output_df['bst_cat_rnk_idx'] = output_df.groupby(['celeb_nm'])['cat_rnk'].transform('idxmin') #add index of best match so we can do below to get associated cat
    output_df['bst_cat'] = output_df.loc[output_df.bst_cat_rnk_idx, 'cat'].values #add best cat


    # remove unneeded cols and duplicate rows
    output_df.drop(['cat','cat_rnk', 'bst_cat_rnk_idx'], axis=1, inplace=True)
    output_df.drop_duplicates(subset='celeb_nm', keep='first', inplace=True)
